...
class LaserMapping : public rclcpp::Node {
   public:
    ...
    // LaserMapping();
    LaserMapping(const std::string &name);
    ~LaserMapping() {
    ...
    void PublishOdometry( rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr pub_odom_aft_mapped);
    void PublishFrameWorld();
    void PublishFrameBody( rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr pub_laser_cloud_body);
    void PublishFrameEffectWorld( rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr pub_laser_cloud_effect_world);
    void Savetrajectory(const std::string &traj_file);
