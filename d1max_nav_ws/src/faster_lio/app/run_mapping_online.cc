...
int main(int argc, char **argv) {
    ...
    // LM is global, so relying on static destruction releases its ROS entities
    // after rmw_zenoh's Tokio TLS has already been torn down. Destroy the node
    // while the middleware runtime is still alive.
    executor.remove_node(LM);
    LM.reset();
