...
try {
 for(const theme of ['dark','light'])for(const [width,height] of [[240,190],[300,220],[420,250]])for(const mode of ['demo','stop','stale','replay','empty']) {
  ...
  const overflow=await page.evaluate(()=>[...document.querySelectorAll('.d1-panel,.d1-lifecycle,.d1-service,.d1-service-button,.d1-service-copy,.d1-estop')].filter(e=>e.scrollWidth>e.clientWidth+2||e.scrollHeight>e.clientHeight+2).map(e=>e.className));
  ...
  if(mode==='stop')assert.equal(await page.locator('.d1-hardware.is-stopped').count(),1);
  const stop=await page.locator('.d1-estop').boundingBox();assert.ok(stop.y+stop.height<=height+1);
  results.push({width,height,theme,mode,overflow:false,twoLifecycleButtons:true});
