...
exec ros2 launch foxglove_bridge foxglove_bridge_launch.xml \
  ...
  topic_whitelist:="['^/(front|rear)_lidar(/imu)?$','^/(front|rear)_camera/image_compressed$','^/tf(_static)?$','^/joint_states$','^/d1max_sdk_bridge/(robot_state|behavior_state|connection_state_text|transition_event|faults|velocity)$','^/d1max/monitor/status$','^/d1max/maps/.*$','^/odom/current_pose$']" \
